...
import { processSubmit } from '../src/utilities/utils';
...
it('should signal an error if email is not in correct format', () => {
    // Any number of characters followed by '@' followed by any number of characters followed by one '.' followed by at least 2 characters
...
it('should signal an error if email is not in correct format', () => {
    // Any number of characters followed by '@' followed by any number of characters followed by one '.' followed by at least 2 characters
    const failData = generateFailTestData('email', 'testUser@test.');
    const [errors, registerInstance] = processSubmit(failData);
    expect(errors).toBe(true);
    expect(registerInstance.email.errors).toHaveLength(1);
    expect(registerInstance.email.errors[0].isError).toBe(true);
...
if('should signal an error if password is < 8 characters', () => {
...
it('should signal an error if all inputs are missing', () => {
    ...
    expect(missingErrors).toBe(true);
});
it('should signal an error if some inputs are missing', () => {
    ...
    expect(usernameErrors[0].errorMsg).toBe(undefined);
});
